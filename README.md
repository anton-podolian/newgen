...
|---|---|---|
| `IMAGE_API_KEY` | Hugging Face token (required) | empty |
| `IMAGE_MODEL` | Hub model ID | `stabilityai/stable-diffusion-xl-base-1.0` |
| `IMAGE_API_URL` | Endpoint template; `{model}` is substituted | HF routed inference URL |
...
3. Under **Variables**, add `IMAGE_API_KEY` and, if needed, `IMAGE_MODEL`. Do not set `PORT`; Railway injects it.
